...
The branch ref shown below is intentionally mutable while the proposal is under
review; consumers that need reproducibility must replace it with the exact commit
SHA they reviewed.
...
An injected route table attached to an `isolated` group is inspected through
`data.aws_route_table` before association. Only AWS local routes and S3/DynamoDB
gateway-endpoint routes are accepted. The physical `route_table_id` must be known
while planning so the module can prove that boundary. A computed ID fails closed;
`isolated_accepts_uninspected_route_table = true` is an explicitly dangerous opt-in
that skips inspection and makes the caller responsible for every pre-existing and
future route in that table.
...
Subnet IPAM is the intentional exception: `secondary_cidr_key` selects the VPC
association dependency, while AWS allocates from the subnet's separate
`ipam_pool_id`. Provider 6.59 exposes pool scope and immediate source-pool metadata
but not an authoritative plan-time link from a future subnet allocation to one
specific VPC CIDR association across arbitrary pool-depth hierarchies. Equality of
pool IDs would reject valid parent/child designs, and matching scopes would accept
sibling pools. The module therefore cannot prove this ancestry without a false
contract; AWS validates allocation compatibility during apply. Callers must ensure
the subnet pool descends from the pool/range represented by the selected secondary.
...
Follow the normative [v5 upgrade guide](docs/UPGRADE-GUIDE-5.0.md). The supporting
...
feature-union moved catalog (the representative migration fixture selected 26 and omitted
37 absent sources), three declarative `removed { destroy=false }` Flow Logs
forgets plus one log-group import and, when v4 IPv6 exists, one association import
in the zero-destroy normal-plan gate, the documented `terraform_data`
precondition-record allowance, and post-apply physical-ID checks. Always test the
procedure against a copy of state and keep the caller-owned `moved.tf` until every
...
| [aws_route_table.isolated_injected](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/data-sources/route_table) | data source |
