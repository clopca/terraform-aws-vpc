...
- a declarative, same-plan ownership handoff for a v4-created CloudWatch log group;
...
flowchart LR
  ...
  Translate --> Handoff[63 moved blocks + removed/import\nin one normal plan]
...
4. Copy the commented root `removed`/`import` blocks from `main.tf`, uncomment them in the caller root, then run the saved normal-plan gate in the upgrade guide. Do not use `terraform apply` on this sample unchanged.
...
v4 configured `name_prefix`; v5 configures the exact fixed `name`. AWS provider 6.59 marks both fields `Optional + Computed + ForceNew`. Its importer reads by physical name and then records both the observed `name` and a derived, provider-computed `name_prefix`; import does **not** clear the prefix. Because v5 configures the exact observed `name` and omits `name_prefix`, no naming replacement is required. A direct move can also converge after a provider-6.x baseline refresh, but the `removed { destroy = false }` plus `import` form makes the ownership handoff explicit and independently refreshes the final address in the same gate plan. The IAM role keeps a normal move and requires its exact v4 `name_prefix`.
