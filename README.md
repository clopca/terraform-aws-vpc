...
This module builds Amazon VPC topologies from a typed subnet contract: subnet
groups with explicit semantic roles, deterministic CIDR allocation, stable
`"<group>/<az>"` resource keys, create-or-inject boundaries for every edge
resource, native Flow Logs, and tiered outputs with a documented stability
contract.

> Upgrading from v4? See the [5.0 upgrade guide](docs/UPGRADE-GUIDE-5.0.md) for
> a declarative migration path (`moved`/`removed`/`import`) with no resource
> replacement.

## Usage

Use explicit AZ names and CIDRs for production.
...
An injected route table cannot be attached to an `isolated` group by default. AWS
provider 6.59 omits propagated and service-managed route classes from
`data.aws_route_table`, so the module cannot exhaustively inspect an unmanaged table
or guarantee isolation. `isolated_accepts_uninspected_route_table = true` is the
only supported path and is an explicitly dangerous opt-in: the caller accepts
responsibility for every existing and future route in that table. This rule is
independent of whether `route_table_id` is known during planning.
