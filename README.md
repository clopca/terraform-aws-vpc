...
| [`enterprise`](examples/enterprise) | Explicit production AZs/CIDRs, a secondary IPv4 range, dual-stack and isolated tiers, one NAT per AZ, Flow Logs, VPC Lattice, and layered tags. | You need a production-oriented multi-tier starting point. | Creates three NAT Gateways, CloudWatch Logs, and VPC Lattice resources. |
...
| [`dual_stack`](examples/dual\_stack) | Caller-keyed IPv6 parent selection, dual-stack and IPv6-native groups, EIGW, DNS64/NAT64, and pinned IPv6 indices. | IPv6 behavior is the focus of your design. | Creates one NAT Gateway. |
| [`existing_vpc`](examples/existing\_vpc) | Injection of a computed VPC, IGW, shared route table, and EIPs while the module owns subnets, associations, routes, and NAT. | You are composing with a network foundation stack. | Self-contained example creates the injected resources and billable NAT Gateways. |
