...
| [`dual_stack`](examples/dual_stack) | Caller-keyed IPv6 parent selection, dual-stack and IPv6-native groups, EIGW, DNS64/NAT64, and pinned IPv6 indices. | IPv6 behavior is the focus of your design. | Creates one NAT Gateway. |
| [`existing_vpc`](examples/existing_vpc) | Injection of a computed VPC, IGW, shared route table, and EIPs while the module owns subnets, associations, routes, and NAT. | You are composing with a network foundation stack. | Self-contained example creates the injected resources and billable NAT Gateways. |
